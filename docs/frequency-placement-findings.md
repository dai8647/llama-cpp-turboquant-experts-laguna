...
# 補足: 動的ページング監査の発見 (feat/prefill-double-buffer 英語版)

feat/prefill-double-buffer ブランチには、上記「発見1〜4」とは独立に q* 配線と
global LRU プールの設計経緯を整理した英語版セクションが含まれていた。
発見の本質は一致しているため、参考用に併記する。
...
`find(layer, expert)` returning the original logical id, which then
appeared as a bank index in the graph. Two complementary measures:
...
## Status (as of feat/prefill-double-buffer)
