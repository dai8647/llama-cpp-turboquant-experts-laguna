    ...
    if (llama_moe_gpu_expert_bank_ensure(model, layer_id, n_experts)) {
        ...
        if (stats) {
            ...
            if (cache.n_copy % 4096 == 0) {
                LLAMA_LOG_INFO("%s: MoE GPU slot stats: copies=%lld hit=%lld miss=%lld evict=%lld residents=%lld cross_evict=%lld qstar_xfer=%lld qstar_cpu=%lld copy=%.1f MiB avg=%.2f ms h2d_gbps=%.2f\n",
                        ...
                        (long long) cache.n_qstar_xfer, (long long) cache.n_qstar_cpu,
    ...
    for (const ggml_tensor * w : { w_in0, w_in1 }) {
        if (w == nullptr || !has_expert_axis(w) ||
            ...
            LLAMA_LOG_WARN("%s: q* host exec unsupported at layer %d: incompatible expert tensor layout "
                    "(w_down ne=[%lld,%lld,%lld,%lld] type=%d, w ne=[%lld,%lld,%lld,%lld] type=%d, n_experts=%d)\n",
                    __func__, layer_id,
                    (long long)w_down->ne[0], (long long)w_down->ne[1], (long long)w_down->ne[2], (long long)w_down->ne[3], (int)w_down->type,
                    (long long)w->ne[0],    (long long)w->ne[1],    (long long)w->ne[2],    (long long)w->ne[3],    (int)w->type,
                    n_experts);
            ex.supported = false;
            return false;
        }
...

        {
            ...
            if (params.moe_gpu_expert_slot_auto) {
                ...
                if (effective_slots <= 0) {
                    ...
                    // q* bandwidth-adaptive policy (opt-in): CLI flags win,
                    // env vars are the fallback so scripts can drive either
                    model->moe_gpu_expert_cache.owner_model = model;
                    if (params.moe_qstar) {
                        model->moe_gpu_expert_cache.qstar_enabled = true;
                    } else if (const char * qs = getenv("LLAMA_MOE_QSTAR")) {
                        model->moe_gpu_expert_cache.qstar_enabled = qs[0] != '\0' && qs[0] != '0';
                    }
                    if (params.moe_qstar_threads > 0) {
                    ...
                    if (model->moe_gpu_expert_cache.qstar_enabled) {
                        LLAMA_LOG_INFO("%s: q* bandwidth-adaptive policy enabled (threads=%d budget=%.0fus)\n",
                                ...
                                model->moe_gpu_expert_cache.qstar_budget_us);
                        llama_moe_gpu_qstar_calibrate(*model);
                    }
                }
            }
