...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    ...
    // n_embd_r() must equal the conv-state row this arch writes:
    //   (ssm_d_conv-1) * (ssm_d_inner + 2*ssm_n_group*ssm_d_state)
    // which requires ssm_d_inner == ssm_dt_rank * ssm_d_state
    if (hparams.ssm_d_inner != hparams.ssm_dt_rank * hparams.ssm_d_state) {
        throw std::runtime_error(format(
            "%s: ssm_d_inner (%u) must equal ssm_dt_rank * ssm_d_state (%u * %u)",
            ml.llm_kv(LLM_KV_SSM_INNER_SIZE).c_str(),
            hparams.ssm_d_inner, hparams.ssm_dt_rank, hparams.ssm_d_state));
    }
    ...
    // optional: MTP-only draft files carry the draft block, trunk files leave it absent
...
ggml_tensor * llama_model_qwen4exp::graph::build_hc_mix(
    ...
    {
        // unfused path: fork lacks ggml_dsv4_hc_pre_gated
...
ggml_tensor * llama_model_qwen4exp::graph::build_hc_combine(
    ...
    // ggml_dsv4_hc_post requires a real comb tensor and HIP has no kernel, so stay unfused.
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn(
    ...
    // (the MTP draft block passes a null hybrid context and always attends dense)
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn_linear(
    ...
    final_output = ggml_cont_2d(ctx0, final_output, head_v_dim * num_v_heads, n_seq_tokens * n_seqs);
    cur = ggml_mul_mat(ctx0, model.layers[il].ssm_out, final_output);
